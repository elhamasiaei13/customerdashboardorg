...
var MIDDLE_SEPERATOR = '\xa0';
var TIME_SEPERATOR = ':';
var EMPTY_VALUE = "    ".concat(DATE_SEPERATOR, "  ").concat(DATE_SEPERATOR, "  ").concat(MIDDLE_SEPERATOR, "  ").concat(TIME_SEPERATOR, "  ");
...
function (_Component) {
  ...
  function DateInput(_props) {
    ...
    _this.readValuesFromProps = function (props) {
      ...
      var valueIsValid = !!j;
      var value = valueIsValid ? (props.gregorian ? formatGregorian(j.value) : formatJalaali(j.j)) + MIDDLE_SEPERATOR + formatTime(j.value) : '';
    ...
    _this.handleBlur = function (event) {
      var splittedValue = splitDateTimeValue(_this.values.value);

      _this.updateState(_this.sanitizeValues(splittedValue, _this.values, true, true, true, true, true));
    ...
    _this.jumpToNext = function () {
      ...
      if (_this.isCaretAtDay(splittedValue, selectionStart)) {
        _this.updateState(_this.sanitizeValues(splittedValue, _this.values, true, false, false, false, false));

        _this.jumpToMonth();

        return true;
      } else if (_this.isCaretAtMonth(splittedValue, selectionStart)) {
        _this.updateState(_this.sanitizeValues(splittedValue, _this.values, false, true, false, false, false));

        _this.jumpToYear();

        return true;
      } else if (_this.isCaretAtYear(splittedValue, selectionStart)) {
        _this.updateState(_this.sanitizeValues(splittedValue, _this.values, false, false, true, false, false));

        _this.jumpToHour();

        return true;
      } else if (_this.isCaretAtHour(splittedValue, selectionStart)) {
        _this.updateState(_this.sanitizeValues(splittedValue, _this.values, false, false, false, true, false));

        _this.jumpToMinute();

        return true;
      } else if (_this.isCaretAtMinute(splittedValue, selectionStart)) {
        _this.updateState(_this.sanitizeValues(splittedValue, _this.values, false, false, false, false, true));
      }
    ...
    _this.jumpToHour = function () {
      _this.values.selectionStart = 13;
      _this.values.selectionEnd = 13;

      _this.inputRef.current.setSelectionRange(_this.values.selectionStart, _this.values.selectionEnd);
    };

    _this.jumpToMinute = function () {
      _this.values.selectionStart = 16;
      _this.values.selectionEnd = 16;
    ...
    _this.stringArrived = function (d) {
      d = mapToLatin(d);
      var date = isValueValidDateTime(d, _this.props.gregorian);
    ...
    _this.updateState = function (newState, noFireOnChange) {
      ...
      if (!_this.values.value) {
        ...
        if (newState.date) {
          ...
          var date = isValueValidDateTime(_this.values.value, _this.props.gregorian);
    ...
    _this.isCaretAtHour = function (splittedValue, selectionStart) {
      return splittedValue && selectionStart <= splittedValue.seperator4 && selectionStart > splittedValue.seperator3;
    };

    _this.isCaretAtMinute = function (splittedValue, selectionStart) {
      return splittedValue && selectionStart > splittedValue.seperator4;
    };

    _this.sanitizeValues = function (splittedValue, values, sanitizeDay, sanitizeMonth, sanitizeYear, sanitizeHour, sanitizeMinute) {
      ...
      var newValue = "".concat(year).concat(DATE_SEPERATOR).concat(month).concat(DATE_SEPERATOR).concat(day).concat(MIDDLE_SEPERATOR).concat(hour).concat(TIME_SEPERATOR).concat(minute);
    ...
    _this.inspectValues = function (values) {
      ...
      } else if (_this.isCaretAtHour(splittedValue, values.selectionStart)) {
        var _inspected3 = inspectHour(hour, values.selectionStart, splittedValue.seperator3);

        newStartPosition = _inspected3.newStartPosition;
        hour = _inspected3.newHour;
      } else if (_this.isCaretAtMinute(splittedValue, values.selectionStart)) {
        var _inspected4 = inspectMinute(minute, values.selectionStart, splittedValue.seperator4);

        newStartPosition = _inspected4.newStartPosition;
        minute = _inspected4.newMinute;
      }
    ...
    _this.deleteValue = function (element, qty) {
      ...
      if (selectionStart === selectionEnd) {
        if (qty < 0) {
          ...
          var toBeDeleted = valueToShow.substring(selectionStart + qty, selectionStart);
          ...
          var _toBeDeleted = valueToShow.substring(selectionStart, selectionStart + qty);

          if (_toBeDeleted === DATE_SEPERATOR || _toBeDeleted === MIDDLE_SEPERATOR || _toBeDeleted === TIME_SEPERATOR) return;
